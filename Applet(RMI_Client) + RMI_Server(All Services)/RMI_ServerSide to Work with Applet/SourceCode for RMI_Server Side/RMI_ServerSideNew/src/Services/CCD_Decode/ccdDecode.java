...
public class ccdDecode {
   ...
   public ccdDecode(String fileName) {
	  try{ 
		...
		//ContinuityOfCareDocument ccdDocumentIn = (ContinuityOfCareDocument) CDAUtil.load(f_in, result );
        ...
        for (PatientRole patient_role: ccdDocumentIn.getPatientRoles() ) {
			...
			if (!patient_role.getAddrs().isEmpty()){
				//type of address could be omitted for output text in terms of this project
				//if (!patient_role.getAddrs().get(0).getUses().isEmpty())// like HP (primary home) could be omitted
        ...
        for (Section section: ccdDocumentIn.getSections()) {
				...
				//System.out.println("Section: "+ section.getTitle().getText());
				//if (section.getTitle().getText().equalsIgnoreCase("Allergies, Adverse Reactions, Alerts")){
